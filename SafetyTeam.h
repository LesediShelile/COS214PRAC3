#ifndef SAFETYTEAM_H
#define SAFETYTEAM_H

#include "EventUnit.h"

/**
 * @brief Concrete EventUnit named SafetyTeam.
 *
 * SafetyTeam participates as a Leaf in the Composite pattern and Observer participant.
 */
class SafetyTeam : public EventUnit {
    public:
        /** @brief Constructs this SafetyTeam. */
        SafetyTeam();
        /** @brief Reacts to a pushed event notice. @param notice Notice received from a Subject. */
        void update(const EventNotice& notice) override;
        void onSafetyAlert() override;
        void onEvacuate() override;

};

#endif
